  ...
  useEffect(() => {
    ...
    if (ethWindow.ethereum) {
      ...
      web3Provider
        ...
        .catch((err) => setStatus(`Error connecting to wallet: ${err.message}`));
    } else {
...
export const useTicketState = (userAddress: string, eventId: string) => {
  ...
  return { hasTicket, setHasTicket, buttonType, setButtonType, buttonText, setButtonText };
};
