...
import { contracts, estimateGas, CONTRACT_ADDRESSES } from '@/utils/dev/contractInit';
...
export const handleTicketPurchase = async ({
  ...
  try {
    ...
    const approveTx = await usdcContract.approve(CONTRACT_ADDRESSES.MuseumAdd, ticketPrice, {
    ...
    const gasLimitPurchase = await estimateGas(museumContract, 'purchaseTicket', [
      eventId,
      ticketPrice
    ]);
    const purchaseTx = await museumContract.purchaseTicket(eventId, ticketPrice, {
      gasLimit: gasLimitPurchase
    });
    const receipt = await purchaseTx.wait(2);

    // Handle success
    await createTicketRecord(receipt, userId);
